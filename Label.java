 ...
 * Purpose: Contains the image and functions required
			to detected the main features in a Lable
			(Colour, Text, Symbol, Class number).
			Extends Thread so it can be multi threaded 
			with other labels in the same image.
 *
 *****************************************************/
...
public class Label extends Thread
{
	...
	private String pThreadName;				//The thread name
	public boolean finished = false;		//The thread finished flag

    private Mat pMat;

	//When the adaptive filter is inversed
    ...
    private String pc2ClassNum = "(none)";	//The class number

	//When the adaptive filter is normal
	...
	private static String checkColourHSV(double[] col)
	{
        ...
        if (col.length == 3)
        {
			...
			//special cases first
			if (150 < hue && hue < 180 && sat < 45 && val < 140)
				s = "Blue";
			//then black
            else if (val <= 30)//40
                s = "Black";
	...
	//A private class which contains the detected txt
	//and the midpoint of the text
	//This point is used to see if it is a valid text or class number
	...
	//This function applys an adaptive threshold to the label and returns a List
	//of all Detected alphanumeric text
	//An adaptive threshold is applied to the v channel of the hsv image
	//This is because the text will either be white or black, the two extremes of the v channel,
	//meaning there will always be a sharp change of the background compared to the text
	//regardless of the text colour or the background colour.
	//Uses blob detection to find the letters/numbers
	private List<DetectedText> detectText(Mat hsvMat, boolean inverse)
	{
		int channel = 3;
		 //threshold the image
		...
		blobDetector.read("Data/blob.xml");	//reads the properties
	...
	//Detects the text for both the normal and inverse adaptive thresholds and sets the
	//correct variables
    public void detect()
    {
        //convert to hsv
	...
	//Checks if a point is a valid class position
	//These are able to be static frames as it is just a general check
    private boolean validClassPosition(Point p)
    {
        boolean flag = false;
        Rect rect = new Rect(181, 343, 166, 126);//181,343, 166,126
        if (rect.contains(p))
            flag = true;
	...
	//This is done as it takes a long time (100ms per image) to do OCR, meaning it is
	//not feasible to detect each text seperately 
	private List<DetectedText> getStrings(Mat mat, KeyPoint[] kp, int maxdistX, int maxdistY)
	{
		...
		for (int i = 0; i < kp.length; i++)
		{
            ...
            //check if it matches a letter already delete it
            for (int j = 0; j < i; j++)
            {
                boolean erase = false;
			...
			//gets rid of any noise
			else if (validClassPosition(kp[i].pt) && ImgProcessing.getNumWhitePixels(letters[i]) < 350 )
			{
				double size = (double)ImgProcessing.getHeight(letters[i])*(double)ImgProcessing.getWidth(letters[i]); //(double)ImgProcessing.getLongestVerticalLine(letters[i])*(double)ImgProcessing.getLongestHorizontalLine(letters[i]);
				double ratio = (double)ImgProcessing.getHeight(letters[i])/(double)ImgProcessing.getWidth(letters[i]);//(double)ImgProcessing.getLongestVerticalLine(letters[i])/(double)ImgProcessing.getLongestHorizontalLine(letters[i]);



				//only if its not a square, otherwise delete it
				if (ratio < 0.9 || ratio > 1.1 || ImgProcessing.getNumWhitePixels(letters[i])/size < 0.75)
				{
					letters[i] = Mat.zeros(1, 1, letters[i].type());
					uncropperLetters[i] = Mat.zeros(1, 1, letters[i].type());
				}
    ...
    //Returns both a cropped [0] and uncropped letter [1]
	//This is used in getStrings to remove duplicates
	private Mat[] getLetter(Mat mat, KeyPoint keypoint)
	{
		...
		//and add them to image as white pixels
		int size = 0;
		int found = 0;
		while (queue.size() != 0)
		{
		...
		//Or if a certain min/max width/height is reached
	...
	//Returns a rectangle that has a boarder of boarder length around it
	//Also accounts for situations when that would put the mat index out of range
	...
	//Returns the text that best suits what is expected of a word
	//No dictionaries are used in this part, but rather relies
	//on the ocr's ability to spit our random things when its fed noise
	private String getBestText()
	{
		...
		//check the amount of spaces (doesnt count double spaces)
		//Invers channel
		int spaces = 0;
		...
		//If both are valid, the text with the least amount of lowercase letters is then chosen
		//If it is still equal, the text with the least amount of numbers is chosen (As there should not be any numbers in the text)
		//if its still equal, chose the one with the longest text

		//If neither are valid, return (none)
	...
	//Compares the class number obtained with the regular adaptive threshold and with the inverse
	//Returns the class num that best suits what is expected of a number
	//No dictionaries are used in this part, but rather relies
	//on the ocr's ability to spit our random things when its fed noise
