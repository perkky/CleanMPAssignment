 ...
 * Picture class
 * Inherits from: nil
 * Purpose: Contains the original image and preforms the 
			neccessary segmentation to retrieve
			the labels within the image.
 *
 *****************************************************/
...
public class Picture
{
    ...
    //Returns true if the Mat is all black or null
    //False otherwise
    public boolean isEmpty()
    {
        boolean flag = true;

        if (Core.countNonZero(ImgProcessing.convertImage(pMat, Imgproc.COLOR_BGR2GRAY)) > 1 || pMat == null)
			flag = true;

        return false;
    }

	//Gets a list containing each identified label.
	//Does this by finding all 4 pointed contours with an area greater than 10000
	//It then extracts this countour and loads it into a label class
    public List<Label> getLabels()
    {
		...
		for (int i = 0; i < contours.size(); i++)
		{
			...
			//extract that label
			Mat shapeMat = extractLabel(pMat, mask, contours.get(i), new Size(500,500));

			//As long as the image is not black, create a label with it.
