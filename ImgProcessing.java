...
public class ImgProcessing
{
	...
	//Returns the amount of white pixels in a single channeled mat
	...
	//Returns the most furthest right non black pixel x value minus the
	//furthest left non black pixel x value
	//Returns the true width of the single channel mat
	...
	//Returns the lowest non black pixel y value minus the
	//highest non black pixel y value
	//Returns the true width of the single channel mat
    ...
    //Contains predetermined constants
    ...
    //Preforms an adaptive threshold on a certain channel - index starts at 1
	//flag is the method used to calculate the threshold - Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C or Imgproc.ADAPTIVE_THRESH_MEAN_C
    ...
    //Preforms a regular threshold on a single channeled image
    ...
    //getChannel - will return a matrix containing a single channels values
	//Will return a mat of zeros if the channel is out of range
    public static Mat getChannel(Mat mat, int channel)
	{
		List<Mat> channels = new ArrayList<Mat>(3);
		Core.split(mat, channels);
		Mat dest = Mat.zeros(1, 1 ,CvType.CV_8U);

		if (channel > 0 && channel <= channels.size())
			dest = channels.get(channel-1);

		return dest;
	}

	//Crops the rectangle out of the inputted mat
    public static Mat cropImage(Mat mat, Rect rect)
    {
        return new Mat(mat, rect);
    }

    //Creates a mask for an image based on a contour
